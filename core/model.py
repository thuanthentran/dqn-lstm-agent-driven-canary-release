...
class TemporalAggregationLayer(nn.Module):
    """Cross-attention over time to aggregate the sequence into a single vector.

    Uses a learnable query to attend to all timesteps.

    Input: (B, T, d_model)
    Output: (B, d_model), temporal_agg_weights (B, n_heads, 1, T)
    """
    ...
    def forward(self, x: torch.Tensor):
        B, T, d = x.shape
        # Expand query for the batch: (B, 1, d)
        query = self.agg_query.expand(B, -1, -1)

        # Cross-attention: Q=(B, 1, d), K=V=(B, T, d)
        out, weights = self.cross_attn(
            query, x, x, average_attn_weights=False
        )
        # out: (B, 1, d) -> (B, d)
        out = self.norm(out.squeeze(1))

        # weights: (B, n_heads, 1, T)
        return out, weights
...
class TransformerFeatureExtractor(BaseFeaturesExtractor):
    ...
    Explainability attributes (updated every forward pass):
        ...
        - last_aggregation_attention: (B, n_heads_agg, 1, T)
    """
    ...
    def __init__(
        ...
        self.last_aggregation_attention: Optional[torch.Tensor] = None
    ...
    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        ...
        self.last_aggregation_attention = agg_attn.detach().cpu()
    ...
    def get_attention_maps(self) -> Dict[str, Optional[np.ndarray]]:
        ...
        Returns:
            dict with keys:
                ...
                - 'temporal_aggregation_attention': np.ndarray (B, n_heads, 1, T) or None
        """
        ...
        aa = self.last_aggregation_attention.numpy() if self.last_aggregation_attention is not None else None
        return {"feature_attention": fa, "temporal_attention": ta, "temporal_aggregation_attention": aa}