...
/**
 * @ingroup examples
 * @file cttc-nr-scenario-isac-contention.cc
 * @brief Scenario 4 (ISACContention): background interference traffic with sinusoidal pattern.
 *
 * Adds a 3rd interference UE with high-rate traffic whose packet interval
 * varies sinusoidally (0.3 + 0.3*sin(t*0.5)^2), competing for PRBs.
 * Only the original lowLat and voice flows are logged in the KPI trace.
 *
 ...
 */
NS_LOG_COMPONENT_DEFINE("CttcNrScenarioIsacContention");
 ...
 * Interval = 1.0 / (baseLambda * factor)
 * @param app the interference UdpClient application
 * @param baseLambda base packet rate
 * @param adjustInterval how often to adjust
 */
void
AdjustInterferenceTraffic(Ptr<Application> app, double baseLambda, double adjustInterval)
{
    ...
    Simulator::Schedule(Seconds(adjustInterval), &AdjustInterferenceTraffic,
                        app, baseLambda, adjustInterval);
}
...
main(int argc, char* argv[])
{
    ...
    uint16_t gNbNum = 1;
    uint16_t ueNumPergNb = 3;  // 2 main UEs + 1 interference UE
    bool logging = false;
    ...
    NodeContainer ueInterferenceContainer;
    ...
    for (uint32_t j = 0; j < gridScenario.GetUserTerminals().GetN(); ++j)
    {
        ...
        {
            ueInterferenceContainer.Add(ue);
        }
    }
    ...
    NetDeviceContainer ueInterferenceNetDev = nrHelper->InstallUeDevice(ueInterferenceContainer, allBwps);

    NetDeviceContainer ueNetDevs;
    ueNetDevs.Add(ueLowLatNetDev);
    ueNetDevs.Add(ueVoiceNetDev);
    ueNetDevs.Add(ueInterferenceNetDev);
    ...
    Ipv4InterfaceContainer ueInterferenceIpIface =
        nrEpcHelper->AssignUeIpv4Address(NetDeviceContainer(ueInterferenceNetDev));

    // attach UEs to the closest gNB
    nrHelper->AttachToClosestGnb(ueLowLatNetDev, gnbNetDev);
    nrHelper->AttachToClosestGnb(ueVoiceNetDev, gnbNetDev);
    nrHelper->AttachToClosestGnb(ueInterferenceNetDev, gnbNetDev);
    ...
    /*
     * Interference traffic: high-rate background traffic on the interference UE.
     * This traffic competes for PRBs but is NOT logged in the KPI trace.
     */
    uint16_t dlPortInterference = 1236;
    double baseLambdaNoise = 20000;

    UdpServerHelper dlPacketSinkInterference(dlPortInterference);
    ApplicationContainer interferenceServerApps;
    interferenceServerApps.Add(dlPacketSinkInterference.Install(ueInterferenceContainer));

    UdpClientHelper dlClientInterference;
    dlClientInterference.SetAttribute("MaxPackets", UintegerValue(0xFFFFFFFF));
    dlClientInterference.SetAttribute("PacketSize", UintegerValue(1252));
    dlClientInterference.SetAttribute("Interval", TimeValue(Seconds(1.0 / baseLambdaNoise)));

    // Use NGBR_LOW_LAT_EMBB for interference traffic (same BWP as lowLat)
    NrQosFlow interferenceFlow(NrQosFlow::NGBR_LOW_LAT_EMBB);
    Ptr<NrQosRule> interferenceRule = Create<NrQosRule>();
    NrQosRule::PacketFilter dlpfInterference;
    dlpfInterference.localPortStart = dlPortInterference;
    dlpfInterference.localPortEnd = dlPortInterference;
    interferenceRule->Add(dlpfInterference);

    ApplicationContainer interferenceClientApps;
    for (uint32_t i = 0; i < ueInterferenceContainer.GetN(); ++i)
    {
        Ptr<NetDevice> ueDevice = ueInterferenceNetDev.Get(i);
        Address ueAddress = ueInterferenceIpIface.GetAddress(i);

        dlClientInterference.SetAttribute(
            "Remote",
            AddressValue(addressUtils::ConvertToSocketAddress(ueAddress, dlPortInterference)));
        interferenceClientApps.Add(dlClientInterference.Install(remoteHost));
        nrHelper->ActivateDedicatedQosFlow(ueDevice, interferenceFlow, interferenceRule);
    }

    interferenceServerApps.Start(udpAppStartTime);
    interferenceClientApps.Start(udpAppStartTime);
    interferenceServerApps.Stop(simTime);
    interferenceClientApps.Stop(simTime);

    // Schedule sinusoidal interference traffic adjustment
    for (uint32_t i = 0; i < interferenceClientApps.GetN(); ++i)
    {
        Simulator::Schedule(udpAppStartTime, &AdjustInterferenceTraffic,
                            interferenceClientApps.Get(i), baseLambdaNoise, 0.05);
    }
